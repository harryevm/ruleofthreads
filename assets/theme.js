(function ($) {
  ...
  window.custom_select = new CustomSelect();
  /**
  ...
  */
  ...
  theme.suffixIds = (container, prefix) => {
    var suffixCandidates = ['id', 'for', 'aria-describedby', 'aria-controls'];var _loop2 = function _loop2(
    i) {
  ...
  */
  var CCCarousel = class extends HTMLElement {
    ...
    init() {
      ...
      this.flexDirection = getComputedStyle(this).getPropertyValue('--flex-direction');

      if (this.scrollArea.childElementCount === 0 || this.flexDirection == 'column') return; // no js handling for vertical column
    ...
    this.functions = {
      ...
      ajaxLoadUrl: function ajaxLoadUrl(url, pushState) {
        ...
        this.currentAjaxLoadUrlFetch = $.get(url, function (data) {
          ...
          if (scrollTo) {
            theme.scrollToRevealElement(scrollTo, true);
          }
        }.bind(this));
      } };
  ...
  var CCProductBlock = class extends HTMLElement {
    ...
    monitorSwatchSelection() {
      if (this.querySelector('[data-media].product-block-options__item')) {
        theme.addDelegateEventListener(this, 'mouseover', '[data-media].product-block-options__item', this.selectSwatch.bind(this));
        theme.addDelegateEventListener(this, 'click', '[data-media].product-block-options__item', this.selectSwatch.bind(this));
      }
    }
    ...
    $(document).on('click', '.quickbuy-toggle', function () {
      ...
      if (pageWidth > 767.98) {
        ...
        if (!qbCont) {
          ...
          qbCont.querySelector('.cc-popup-close').addEventListener('click', () => popup.close());
          qbCont.querySelector('.cc-popup-background').addEventListener('click', () => popup.close());
        ...
        if (!$quickbuyCont.hasClass('quickbuy-loaded')) {
          ...
          $.get(productUrl, function (response) {
            ...
            $newDetail.find('.gallery .thumbnails').addClass('hidden');
